...
user = {'username': ADMIN_USERNAME, 'password': ADMIN_PASSWORD}  # 从config文件中导入管理员用户名和密码
...
def login():
    ...
    if request.method == 'POST':
        ...
        user = query_user(username)  # 判断是否为管理员用户名，如果是则返回管理员用户
        if user is not None and request.form['password'] == user['password']:
            curr_user = User()
            curr_user.id = username

            login_user(curr_user)  # 这里都是用flask_login实现的
